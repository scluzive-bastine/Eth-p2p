    ...
    <div class="row">
        <div class="col-12 col-sm-12 col-md-12 col-lg-8">
            <div>
                ...
                <a href="" class="text-white text-lg">{{$seller->user->name}}</a>
                <p>
                    {{$seller->user->blurb}}
                </p>
                ...
                <div class="mt-3 custom__trade--card-footer">
                    <div>
                        <span><b>Price:</b> {{config('currencies')[$seller->currency]['symbol']}}{{number_format($seller->price, 2)}}</span>
                    </div> 

                    <div>
                        <span><b>Available:</b> {{$seller->balance}} {{$seller->coin}}</span>
                    </div> 

                    <div>
                        <span><b>Limit:</b> {{config('currencies')[$seller->currency]['symbol']}}{{number_format(convertToFiat($seller->coin, $seller->min, $seller->price))}} - {{config('currencies')[$seller->currency]['symbol']}}{{number_format(convertToFiat($seller->coin, $seller->balance, $seller->price))}}</span>
                    </div> 
                    <br>
                    ...
                    <span>
                        <i class="flag flag-gh" style="height: 0.875rem; vertical-align:middle;"></i>
                        Lagos
                    </span>
                    <div class="d-flex mt-2">
                        ...
                        5.0 (1901 reviews) working working
                    </div> --}}
                </div>
        ...
        <div class="col-12 col-sm-12 col-md-12 col-lg-4 d-flex justify-content-lg-end align-items-center">
            <div class="mt-3">
                <h4>{{config('currencies')[$seller->currency]['symbol']}}{{number_format(convertToFiat($seller->coin, $seller->balance, $seller->price))}}</h4>
                <a href="{{route('market.openBuyTrade', $seller->id)}}" class="btn btn-primary btn-lg btn-block rounded-pill">Buy</a>
            </div>
        </div>
    </div>
</div>
