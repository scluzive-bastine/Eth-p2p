    ...
    <div class="row">
        <div class="col-12 col-sm-12 col-md-12 col-lg-8">
            <div>
                ...
                <a href="" class="text-white text-lg">{{$buyer->user->name}}</a>
                <p>
                    {{$buyer->user->blurb}}
                </p>
                <div class="mt-3 custom__trade--card-footer">
                    ...
                    <div>
                        <span><b>Price:</b> {{config('currencies')[$buyer->currency]['symbol']}}{{number_format($buyer->price, 2)}}</span>
                    </div> 

                    <div>
                        <span><b>Total:</b> {{$buyer->balance}} {{$buyer->coin}}</span>
                    </div> 

                    <div>
                        <span><b>Limit:</b> {{config('currencies')[$buyer->currency]['symbol']}}{{number_format(convertToFiat($buyer->coin, $buyer->min, $buyer->price))}} - {{config('currencies')[$buyer->currency]['symbol']}}{{number_format(convertToFiat($buyer->coin, $buyer->balance, $buyer->price))}}</span>
                    </div> 
                    <br>
        ...
        <div class="col-12 col-sm-12 col-md-12 col-lg-4 d-flex justify-content-lg-end align-items-center">
            <div class="mt-3">
                <h4>{{config('currencies')[$buyer->currency]['symbol']}}{{number_format(convertToFiat($buyer->coin, $buyer->balance, $buyer->price))}}</h4>
                <a href="{{route('market.openSellTrade', $buyer->id)}}" class="btn btn-danger btn-lg btn-block rounded-pill">Sell</a>
            </div>
        </div>
    </div>
</div>
