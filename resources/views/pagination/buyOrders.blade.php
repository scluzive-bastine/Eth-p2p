    ...
    @foreach($orders as $order)
        <tr class='text-center'>
            ...
            <td>
                ...
                <div>
                    <a href="{{route('market.sell_request', $order->id)}}" class="text-success">view</a>
                </div> 
            </td>
            <td>
                <button data-toggle="modal" data-target="#editBuyOrder{{$order->id}}" class="btn btn-info btn-sm">Edit</button>

                <form action="{{route('market.deleteBuyOrder', $order->id)}}" method="POST" style="display: inline-block">
                    @csrf 
                    @method('delete')
                ...
                <div class="modal" id="editBuyOrder{{$order->id}}">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content modal-content-demo">
                            <div class="modal-body text-left" >
                                <h4 class="mb-1">Update Order</h4>
                                <form class="updateBuyOrder">
                                    <div class="row">
                                        <div class="col-md-6">
...
<script>
    $(".updateBuyOrder").on('submit', function(e){
        e.preventDefault();
        let form = this;
        ...
        $.ajax({
            url:"{{route('market.updateBuyOrder')}}",
            type:"post",
            data:$(form).serialize(),
